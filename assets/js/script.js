  ...
  if (previousSearch !== null) {
    ...
    for (let i = 0; i < previousSearch.length; i++) {
      ...
      searchButton.addEventListener("click", (event) => {
        ...
        weatherDisplay(event.target.dataset.city);
      });
    }
  }
};
        ...
        fetch(oneCallUrl).then(function (response) {
          if (response.ok) {
            ...
            response.json().then(function (data) {
              ...
              // Clears list from past searches

              currentConditionsUl.innerHTML = "";

              // creates li to hold current weather input
              for (let i = 0; i < 4; i++) {
              ...
              // Removes past cards for 5-Day Forecast container

              dailyCardContainer.innerHTML = "";

              // For Loop to populate cards for next 5 days with information from daily openCall property
